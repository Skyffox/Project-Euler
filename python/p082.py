...
def minimal_path_sum(grid):
    ...
    # Initialize the priority queue with all elements of the first row
    for j in range(n):
        heapq.heappush(pq, (grid[0][j], 0, j))  # (cost, row, col)
    
    # Create a distance table to store minimal cost to each cell
    dist = [[float('inf')] * n for _ in range(n)] # NOTE: different from 81 and 83 because we initialize the entire left column as starting points
    for j in range(n):
        dist[0][j] = grid[0][j]  # Starting from the top row
    
    # Directions for moving in 4 directions: right, down, left, up
    directions = [(0, 1), (1, 0), (0, -1)] # NOTE: only works with right down up
    # Dijkstra's algorithm to find the minimal path sum
    while pq:
        current_dist, row, col = heapq.heappop(pq)
        
        # If we are in the last row, we can check the result
        if row == n - 1:
            return current_dist
